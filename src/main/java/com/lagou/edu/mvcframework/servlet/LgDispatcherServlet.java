...
//        method.invoke() //
        ...
        if (handler.getMethod().isAnnotationPresent(LagouSecurity.class)) {
            //方法权限
            String[] values = handler.getMethod().getAnnotation(LagouSecurity.class).value();
            //用户名
            String username = req.getParameter("username");
            if (!Arrays.asList(values).contains(username)) {
                resp.getWriter().write(username + ",not security ");
                return;
            }
        }
